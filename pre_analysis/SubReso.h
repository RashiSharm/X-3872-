...
// Thu Dec 14 09:17:57 2023 by ROOT version 6.20/04
// from TTree nTuple/nTuple
// found on file: SEntriesM2.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   TBranch        *b_isMaxptmin;   //!

   SubReso(TTree *tree=0);
   virtual ~SubReso();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(TString outfile="subreso_cc.root");
...
#ifdef SubReso_cxx
SubReso::SubReso(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("SEntries_cc.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("SEntries_cc.root");
      }
...
Int_t SubReso::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t SubReso::LoadTree(Long64_t entry)
{
...
void SubReso::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("isMaxptmin", &isMaxptmin, &b_isMaxptmin);
   Notify();
}

Bool_t SubReso::Notify()
{
...
void SubReso::Show(Long64_t entry)
{
...
Int_t SubReso::Cut(Long64_t entry)
{
